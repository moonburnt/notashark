...
async def status_updater():
    ...
    if raw_data and (int(raw_data["total_players_amount"]) > 0):
        message = (
            f"with {raw_data['total_players_amount']} peasants | "
...
async def serverlist_autoupdater():
    ...
    for item in settings_fetcher.SETTINGS_DICTIONARY:
        ...
        try:
            ...
            channel_id = settings_fetcher.SETTINGS_DICTIONARY[item][
                "serverlist_channel_id"
            ]
            channel = bot.get_channel(channel_id)
            message_id = settings_fetcher.SETTINGS_DICTIONARY[item][
...
async def info(ctx, *args):
    ...
    try:
        ...
        await ctx.channel.send(
            f"Couldnt find such server. Are you sure "
        ...
        log.info(
            f"{ctx.author} has asked for server info of {server_address}. Responded"
        )
...
async def set(ctx, *args):
    ...
    if (
        ...
        try:
            ...
            settings_fetcher.SETTINGS_DICTIONARY[str(ctx.guild.id)][
                "serverlist_channel_id"
            ] = channel_id
            ...
            log.error(f"Got exception while trying to edit serverlist message: {e}")
            await ctx.channel.send(
                "Something went wrong... " "Please double-check syntax and try again"
            )
        else:
