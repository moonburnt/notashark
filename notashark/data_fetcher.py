    ...
    if known_server_countries:
        for item in known_server_countries:
            if item["ip"] == raw_data["IPv4Address"]:
                ...
                log.debug(
                    f"{raw_data['IPv4Address']}'s country is "
...
def single_server_fetcher(ip, port):
    ...
    log.debug(
        "Fetching detailed info for server with" f"address {ip}:{port} from kag api"
    )
...
def _serverlist_autoupdater():
    ...
    while True:
        ...
        log.debug(
            f"Waiting {SERVERLIST_UPDATE_TIME} seconds " "to update kag_servers again"
        )
